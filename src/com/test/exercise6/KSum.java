...
public class KSum {
    ...
    private static void findK(int[] nums, int sum, int k) {
        List<List<Integer>> lists = new ArrayList<>();
        List<Integer> list = new ArrayList<>();
        helpK(nums, sum, k, lists, list, 0);
        System.out.print(lists);
    }


    private static void helpK(int[] nums, int sum, int k,
                              List<List<Integer>> lists, List<Integer> list, int pos) {
        ...
        for (int i = pos; i < nums.length; i++) {
            list.add(nums[i]);
            helpK(nums, sum - nums[i], k, lists, list, i + 1);
